...
# --- 2. LOGIC CORE (UPDATED V11.1 - ANTI BIAS) ---
...
def hitung_triangle_v11(result_str):
    """Logika Triangle diperkuat dengan rotasi angka agar tidak kaku"""
    try:
        n = [int(d) for d in str(result_str) if d.isdigit()]
        if len(n) >= 5:
            # Perbaikan: Offset dinamis agar angka tidak selalu sama dengan result
            v1 = (n[0] + n[2] + 5) % 10 
            v2 = (n[1] + n[4] + 7) % 10 
            v3 = (n[-1] * 7) % 10 
...
def generate_weighted_pool(invest_str, bom_str, history_str):
    """
    PERBAIKAN BOBOT: 
    - History diperkuat (3x) agar mendeteksi angka telat.
    - Invest & BOM diseimbangkan (5x) agar tidak bias dominan.
    """
    pool = list(invest_str) * 5 + list(bom_str) * 5 + list(history_str[-60:]) * 3
    
    # Mistik/Indeks Map diperkuat untuk angka cadangan
    mistik_map = {'0':'7','1':'0','7':'0','4':'9','9':'4','5':'8','8':'5','2':'5','3':'8','6':'9'}
    for char in (invest_str + bom_str):
        if char in mistik_map: 
            pool.append(mistik_map[char])
            pool.append(mistik_map[char]) # Ekstra 2x untuk angka bayangan
...
if db:
    ...
    with tab1:
        ...
        with c2: a_in = st.text_input("Angka Result 5D:", placeholder="Contoh: 75465")
    ...
    with tab3:
        ...
        try:
            ...
            res_terakhir = str(df_an['Angka'].iloc[-1]) if not df_an.empty else "75465"
            ...
            # Logic Investasi yang lebih variatif
            ...
            for i in range(1, 4):
                ...
                while len(snip_list) < 10:
                    ...
                    # Anti-Twin Filter untuk non-Red Signal
                    if len(set(res_snip)) == 1 and msg != "🚨 SINYAL MERAH: AWAS TWIN GILA!":
                        continue
            ...
            st.markdown("<p style='font-size:12px;'>Sistem V11.1: Database sinkron, bobot history diperkuat.</p>", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

        except Exception as e: 
            st.warning(f"Error: {e}. Pastikan data di Google Sheets sudah benar.")

    with tab4:
        st.subheader("🔄 BBFS Ultra Smart Pangkas")
        b_in = st.text_input("Masukkan Angka BBFS (Contoh: 12345):")
        ...
        if pangkas_btn and b_in:
            ...
            if len(pool_b) >= target_pangkas:
                ...
                intent_limit = 0
                while len(hasil_bbfs) < 15 and intent_limit < 10000:
